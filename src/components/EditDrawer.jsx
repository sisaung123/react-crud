...
const EditDrawer = () => {
  const {
    editDrawer,
    toggleEditDrawer,
    editCourse: { id, course_title, short_name, fee },
    updateCourse,
  ...
  useEffect(() => {
    (titleRef.current.value = course_title),
  ...
  const handleUpdateBtn = async (event) => {
    ...
    const res = await courseApi.put(
      `/courses/${id} `,
      JSON.stringify(newUpdateCourse)
    );
            ...
            <div className="flex items-center justify-between">
              ...
              <div>
                <button
                  type="submit"
                  disabled={isLoading}
                  className="relative group disabled:ps-8 disabled:opacity-50 disabled:cursor-not-allowed text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                >
